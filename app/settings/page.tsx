  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex gap-3 mb-8">
        <button
          onClick={() => handleTrigger('enviar')}
          disabled={triggerLoading === 'enviar'}
        ...
        <button
          onClick={() => handleTrigger('fups')}
          disabled={triggerLoading === 'fups'}
      ...
      <div className="space-y-4">
        {painel.map((row, idx) => {
          ...
          return (
            <div key={idx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md" style={{ animationDelay: `${idx * 100}ms` }}>
              {/* Header */}
                ...
                <div className="ml-auto">
                  {confirmClear === row.category ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleClearBase(row.category)}
                        disabled={clearing === row.category}
              ...
              <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                <SettingField label="Responsável (email)" value={row.responsavel} onChange={v => updateField(idx, 'responsavel', v)} />
                ...
                <SettingField label="Dias até FUP1" value={row.diasFup1.toString()} onChange={v => updateField(idx, 'diasFup1', parseInt(v) || 3)} type="number" />
                <SettingField label="Dias até FUP2" value={row.diasFup2.toString()} onChange={v => updateField(idx, 'diasFup2', parseInt(v) || 7)} type="number" />
              </div>
            </div>
          );
        })}
...
**Ctrl+S**. Agora no terminal:
```
git add .
```
```
git commit -m "Add clear base, triggers, stats per category"
```
```
git push