...
export async function sendEmail(
  ...
  if (!accessToken) {
    return { success: false, error: `Token não encontrado para ${senderEmail}. Precisa fazer login.` };
  }

  const raw = createRawEmail(senderEmail, to, subject, htmlBody, cc);
...
export async function sendReply(
  ...
  const accessToken = await getValidAccessToken(senderEmail, spreadsheetId);
  if (!accessToken) {
    return { success: false, error: `Token não encontrado para ${senderEmail}` };
  }

  const raw = createRawEmail(senderEmail, to, subject, htmlBody, cc, threadId, originalMessageId);
...
export async function checkReplies(
  ...
  const accessToken = await getValidAccessToken(senderEmail, spreadsheetId);
  if (!accessToken) {
    return { hasReply: false, error: `Token não encontrado para ${senderEmail}` };
  }
